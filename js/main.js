/*
Coin Clicker Compatibility Debugger
*/
...
function supportsNavData() { if (navigator.userAgentData != undefined) return true; else return false; }
...
const sfx2 = document.getElementById('sfx2'), //Shop Unlock
  ...
  buildInfo = { BuildStr: 'compatdbgr', BuildNum: Number.MAX_VALUE, UpdName: 'debugger', UpdNum: null },
...
function updateScreen() {
  try {
    document.title = `Coin Clicker Compatibility Debugger`;
    if (buff == 'none' && init.GameStarted) { stats.RawClicksPS = stats.ClicksPS; stats.RawClickVal = stats.ClickValue; }
    if (!document.hidden) {
      bgm.volume = volume;
